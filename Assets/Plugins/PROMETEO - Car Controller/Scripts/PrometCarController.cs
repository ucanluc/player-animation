...
public class PrometCarController : MonoBehaviour
{
    public bool disableControls;

    //CAR SETUP
    public Vector3 desiredDir = Vector3.zero;
    public bool doHandbrake;
    public bool doHandbrakeLast;
    ...
    */
    ...
    private void Update()
    {
        ...
        */
        if (!disableControls)
        {
            ...
            {
                TurnByValue(desiredSteerAngle);
            }


            if (doHandbrake) // space, handbrake
            {
                CancelInvoke("DecelerateCar");
                deceleratingCar = false;
                Handbrake();
            ...
            if (!doHandbrake && doHandbrakeLast) // space, handbrake released
                RecoverTraction();

            if (desiredDir.z == 0) // forward/backward is zero
                ThrottleOff();

            if (desiredDir.z == 0 && !doHandbrake &&
                !deceleratingCar) // forward/backward is zero and handbrake is off
            {
                InvokeRepeating("DecelerateCar", 0f, 0.1f);
                deceleratingCar = true;
            }

            if (desiredDir.x == 0f && steeringAxis != 0f) // left/right is zero
                ResetSteeringAngle();

            doHandbrakeLast = doHandbrake;
        }
    ...
    public void CarSpeedUI()
    {
        if (useUI)
            try
            {
                var absoluteCarSpeed = Mathf.Abs(carRigidbody.velocity.magnitude);

                // convert absoluteCarSpeed from meters/second to kilometers/hour
                var carSpeedAsKmph = absoluteCarSpeed * 3.6f;
                carSpeedText.text = $"{Mathf.RoundToInt(carSpeedAsKmph)} ({Mathf.RoundToInt(absoluteCarSpeed)})";
            }
            catch (Exception ex)
            {
                Debug.LogWarning(ex);
        ...
        if (useSounds)
        {
            try
            {
                if (carEngineSound != null)
                {
                    ...
                    {
                        carEngineSound.pitch = Mathf.Lerp(carEngineSound.pitch, engineSoundPitch, 0.5f);
                        ;
                    }
                    else
                    {
                        carEngineSound.pitch = Mathf.Lerp(carEngineSound.pitch, initialCarEngineSoundPitch, 0.5f);
                    }
                }

                if (WheelsAreGrounded() && (isDrifting || (isTractionLocked && Mathf.Abs(carSpeed) > 12f)))
                {
    ...
    private void TurnByValue(float desiredSteerAngle)
    {
        var steeringAngle = Mathf.Clamp(desiredSteerAngle, -maxSteeringAngle, maxSteeringAngle);
        frontLeftCollider.steerAngle = Mathf.Lerp(frontLeftCollider.steerAngle, steeringAngle, steeringSpeed);
        frontRightCollider.steerAngle = Mathf.Lerp(frontRightCollider.steerAngle, steeringAngle, steeringSpeed);

        // update steering axis from the steering angle
        steeringAxis = steeringAngle / maxSteeringAngle;
    }
    ...
    public void DecelerateCar()
    {
        ...
        if (carRigidbody.velocity.magnitude < 0.25f * Time.fixedDeltaTime)
        {
    ...
    public void DriftCarPS()
    {
        if (useEffects)
        {
            try
            {
                if (WheelsAreGrounded() && isDrifting)
                {
            ...
            try
            {
                if (WheelsAreGrounded() && (isTractionLocked || Mathf.Abs(localVelocityX) > 5f) &&
    ...
    public bool WheelsAreGrounded()
    {
        return rearLeftCollider.isGrounded && rearRightCollider.isGrounded;
    }
    ...
    public void UpdateDesiredMovement(float forward, float steering,bool handbrake)
    {
        desiredDir.x = steering;
        desiredDir.y = 0f;
        desiredDir.z = forward;
        doHandbrake = handbrake;
